...
This setup document will show how to get `EasyFood` running on your local machine for further development or local dmemos. This documentation will show the minimal setup needed to run the project with or without Docker.

## Requirements

There are two ways to run the project currently. The recommended approach is Docker as its minimal, works regardless of what environment (OS, Node version etc) that you may have, and more importantly, creates a Redis container without local installation. You may use your own local Node, Python and Redis if you wish, but setup of Redis is tedious if you do not have already installed.
...
Once verification of the tools are successful, then you are ready to install and run EasyFood

## Before installation

### 1. Clone the project

Clone EasyFood using Git.
...
Congrats! You now have installed EasyFood and should be able to use it.
